...
.face {
  ...
  background-color: #1e1e24; /* internal/black body */
  display: flex;
  justify-content: center;
  align-items: center;
...
/* Sticker colors */
.color-yellow { background-color: #ffd60a !important; }
.color-white { background-color: #ffffff !important; }
.color-orange { background-color: #ff9f0a !important; }
.color-red { background-color: #ff453a !important; }
.color-green { background-color: #30d158 !important; }
.color-blue { background-color: #0a84ff !important; }
.color-black { background-color: #1c1c1e !important; }
  ...
  <div class="cube-workspace">
    ...
    <div class="cube-right-panel">
      ...
      <div class="tab-content" id="tab-intro">
        ...
        <p>Hãy bấm thử công thức "Sexy Move" kinh điển dưới đây để xem khối Rubik xoay mẫu:</p>
        <div class="algo-player">
          <button class="btn btn-success play-algo" data-algo="R U R' U'">Xem "Sexy Move" (R U R' U')</button>
        </div>
      </div>
...
(function() {
  ...
  function makeMove(moveCode, callback) {
    ...
    // Choose which cubies to rotate and rotation configurations
    switch (baseMove) {
      case 'R':
        filterFunc = c => c.x === 1;
        rotationAxis = 'X';
    ...
    // Animate wrapper
    wrapper.style.transition = 'transform 0.35s cubic-bezier(0.25, 1, 0.5, 1)';
    // Force reflow
    wrapper.offsetHeight;
    
    wrapper.style.transform = `rotate${rotationAxis}(${angleDegrees}deg)`;
    ...
    setTimeout(() => {
      ...
      rotatingCubies.forEach(c => {
        ...
        if (rotationAxis === 'X') {
          ...
          if (angleDegrees === -90) { // R, L'
            c.y = -z;
            c.z = y;
            // Face colors cycle: u <- f <- d <- b <- u
            temp = c.faces.u;
            c.faces.u = c.faces.f;
            c.faces.f = c.faces.d;
            c.faces.d = c.faces.b;
          ...
          if (angleDegrees === -90) { // U, D'
            c.x = -z;
            c.z = x;
            // Face colors cycle: l <- f <- r <- b <- l
            temp = c.faces.l;
            c.faces.l = c.faces.f;
            c.faces.f = c.faces.r;
            c.faces.r = c.faces.b;
          ...
          if (angleDegrees === -90) { // F, f, B'
            c.x = -y;
            c.y = x;
            // Face colors cycle: u <- l <- d <- r <- u
            temp = c.faces.u;
            c.faces.u = c.faces.l;
            c.faces.l = c.faces.d;
            c.faces.d = c.faces.r;
